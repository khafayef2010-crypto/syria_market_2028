// ==============================================================================
// منصة سوق سوريا الشامل 2028 - الكود الصريح الكامل (Production-Ready 100%)
// [الدفعة الأولى 1/3: التهيئة، المفاتيح الحقيقية، النماذج، المصادقة، والهيكل المتجاوب الآمن]
// ==============================================================================
...
// ==============================================================================
// 2. أدوات المساعدة وفحص الأرقام والتنسيق المتجاوب
// ==============================================================================
class PhoneHelper {
...
// ==============================================================================
// 3. نماذج البيانات الحقيقية المربوطة بجداول Supabase
// ==============================================================================
class Moderator {
  final String id;
  final String email;
...
class SponsoredBanner {
  ...
  final int sideIndex; // 0 for Right Carousel, 1 for Left Carousel
  final bool isActive;
...
// ==============================================================================
// 4. خدمة التخزين السحابي الحقيقية لـ Supabase
// ==============================================================================
...
// ==============================================================================
// 5. مدير الحالة وفحص الجلسة وصلاحيات الإدارة (AppStateManager)
// ==============================================================================
class AppStateManager extends ChangeNotifier {
  ...
  // إعدادات وقت تقليب البانرات الممولة المزدوجة (من 1 إلى 5 ثوانٍ)
  int dualBannerIntervalSeconds = 3;
  bool isMaintenanceMode = false;
  String maintenanceMessage =
      'المنصة تخضع لعمليات صيانة مجدولة. سنعود للعمل قريباً جداً!';
  ...
  // فحص الصلاحيات الإدارية الحصرية لـ sameraoaad@gmail.com و aoaadabdo@gmail.com
  ...
  void _initDefaults() {
    subscriptionPlans = [
      SubscriptionPlan(
        ...
        features: [
          'نشر حتى 5 إعلانات شهرياً',
          'حتى 4 صور لكل إعلان',
          'دعم المحادثة والتفاوض المباشر',
        ],
      ),
      SubscriptionPlan(
        id: 'gold_vip',
        ...
        features: [
          'نشر غير محدود للإعلانات',
          'حتى 12 صورة عالية الدقة لكل إعلان',
          'شارة التاج VIP والظهور الدائم بالقمة',
          'دعم فني وتواصل مخصص على مدار الساعة',
        ],
      ),
    ];
...
// ==============================================================================
// منصة سوق سوريا الشامل 2028 - الكود الصريح الكامل (Production-Ready 100%)
// [الدفعة الثانية 2/3: شريط الإعلانات الممولة المزدوج، صفحة تفاصيل الإعلان الممول، نشر الإعلانات، والشاشة الرئيسية]
// ==============================================================================

// ==============================================================================
// 7. صفحة تفاصيل الإعلان الممول المستقلة (SponsoredBannerDetailsScreen)
// ==============================================================================
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  // أزرار التواصل المباشر مع صاحب الإعلان الممول
                  Row(
                    children: [
...
// ==============================================================================
// 8. شاشة نشر إعلان جديد الحقيقية (FullAddAdScreen)
// ==============================================================================
  ...
  Future<void> _pickImage() async {
    ...
    if (_pickedImagesBytes.length >= plan.maxImagesPerAd) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'حد خطتك الحالي هو ${plan.maxImagesPerAd} صور لكل إعلان.',
          ),
        ),
      );
      return;
  ...
  Future<void> _submitAd() async {
    ...
    try {
      ...
      final newAd = AdItem(
        ...
        allowComments: true,
        status: 'approved', // نشر فوري مباشر
        viewsCount: 1,
        sellerRating: 5.0,
...
// ==============================================================================
// 9. كلاس التطبيق الرئيسي (SyriaMarket2028App)
// ==============================================================================
...
class _MainDashboardScreenState extends State<MainDashboardScreen> {
  ...
  // مؤقت تقليب البانرات المزدوجة التلقائي
  ...
  // الجلب الحقيقي من جداول ads و banners في Supabase
  Future<void> _initLiveAdsAndBannersFromSupabase() async {
    ...
    try {
      ...
      // 2. جلب الإعلانات الممولة للجهتين (يمين ويسار)
      ...
      debugPrint('Fetch ads and banners note: $e');
    } finally {
  ...
  // 4. قسم الإعلانات الممولة المزدوج (صورتين بجانب بعض بعرض الشاشة مع تقليب تلقائي)
    ...
    return PageView.builder(
      ...
      itemBuilder: (ctx, idx) {
        ...
        return GestureDetector(
          onTap: () {
            // الانتقال لصفحة تفاصيل الإعلان الممول المستقلة
            Navigator.push(
              context,
              MaterialPageRoute(
    ...
    if (!_manager.isLoggedIn) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.lock_outline_rounded, size: 54, color: Colors.grey),
            const SizedBox(height: 12),
            const Text(
...
// ==============================================================================
// 11. شاشة تفاصيل الإعلان الكاملة (FullAdDetailsScreen)
// ==============================================================================
...
// ==============================================================================
// 12. لوحة تحكم المسؤولين المتقدمة الحصرية (FullAdminPanelScreen)
// ==============================================================================
...
class _FullAdminPanelScreenState extends State<FullAdminPanelScreen>
  ...
  int _selectedSideIndex = 0; // 0 for Right Carousel, 1 for Left Carousel
  ...
  Future<void> _submitSponsoredBanner() async {
    ...
    try {
      // رفع الصورة الحقيقية إلى Supabase Storage في باكت banner-images
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: TabBarView(
          controller: _tabController,
          children: [
            // 1. استمارة رفع ونشر الإعلانات الممولة
            ListView(
              padding: const EdgeInsets.all(16),
              children: [
                const Text(
                ...
                TextField(
                  ...
                  maxLines: 3,
                  decoration: const InputDecoration(
                    labelText: 'التفاصيل الكاملة للإعلان الممول (تظهر في صفحته المستقلة)',
                    filled: true,
                    fillColor: Colors.white,
                  ),
                ),
            ...
            // 2. التحكم الزمني بسرعة تقليب البانرات (من 1 إلى 5 ثوانٍ)
            Padding(
              padding: const EdgeInsets.all(20),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
...
// ==============================================================================
// 13. دالة البدء والتهيئة الشاملة للتطبيق (main)
// ==============================================================================
void main() async {
